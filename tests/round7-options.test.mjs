...
const options=(source,name)=>{
 const match=source.match(new RegExp('const '+name+'(?:\\s*:[^=;]+)? = ([\\s\\S]*?);'));
 assert.ok(match,'Missing option list: '+name);
 return [...Function('EMPLOYMENT_TYPE_OPTIONS','return '+match[1])(EMPLOYMENT_TYPE_OPTIONS)];
};
...
test('employment choices in create, edit and discovery agree',()=>{
 const createField=read('app/org/dashboard/jobs/new/page.tsx').split('<FormField label="Employment Type">')[1].split('</Select>')[0];
 const created=(createField.includes('EMPLOYMENT_TYPE_OPTIONS.map') ? [...EMPLOYMENT_TYPE_OPTIONS] : [...createField.matchAll(/<option>(Full-time|Part-time|Contract|Temporary|Seasonal|Internship|Volunteer|Casual)<\/option>/g)].map(m=>m[1])).sort();
