...
test('profile wizard: accessible selection, explained requirements and profile-only receipt', { skip: !chrome && 'Install Chrome or Playwright Chromium to run the isolated browser fixture' }, async t => {
  ...
  const storage = write('storage.js', `export const storage={}; export const ref=()=>({}); export const uploadBytes=async()=>{await new Promise(resolve=>{window.finishFixtureUpload=resolve;});}; export const uploadBytesResumable=(storageRef,file,metadata)=>{const task={snapshot:{ref:storageRef,bytesTransferred:0,totalBytes:1},on:(event,next,error,complete)=>{if(event!=='state_changed')return()=>{};next({...task.snapshot});new Promise(resolve=>{window.finishFixtureUpload=resolve;}).then(()=>{task.snapshot.bytesTransferred=1;next({...task.snapshot});complete();});return()=>{};}};return task;}; export const getDownloadURL=async()=>'https://fixture.invalid/resume.pdf'; export const getBlob=async()=>{throw Error('Unexpected saved resume read');};`);
