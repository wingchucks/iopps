...
for (const kind of ['manual', 'cron', 'batch']) {
  test(`${kind} actual ingest handler classifies new imported roles while preserving source and review metadata offline`, async () => {
 ...
 if(kind!=='batch')store.rows.set('employers/fixture-org',{standardPostCredits:1});
    ...
    const options = {
      ...
      mocks: {
        ...
        '@/lib/server/feed-source': { ...actualFeed, loadFeedItems: async () => [{ guid: 'fixture', title: 'Registered Nurse', link: 'https://example.test/job', description: raw }] },
      },
    };
    ...
    const request = new Request('https://fixture.test/api/import', { method: kind === 'cron' ? 'GET' : 'POST', headers: { authorization: 'Bearer offline-fixture', 'x-cron-secret': 'offline-fixture', 'content-type': 'application/json' }, ...(kind === 'batch' ? { body: JSON.stringify({ jobs: [{ title: 'Registered Nurse', externalUrl: 'https://example.test/job', description: raw }] }) } : {}) });
    ...
    assert.equal(writes[0].category, 'Health & Wellness');
