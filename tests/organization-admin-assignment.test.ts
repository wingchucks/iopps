...
test('BATC branch cannot auto deploy and existing deployment/crons settings remain exact', () => {
  ...
  assert.equal(config.git.deploymentEnabled['fix/employer-qa-20260920'], false);
  delete config.git.deploymentEnabled['fix/employer-qa-20260920'];
  assert.equal(config.git.deploymentEnabled['fix/master-employer-login-race'], false);
  delete config.git.deploymentEnabled['fix/master-employer-login-race'];
  assert.equal(config.git.deploymentEnabled['fix/individual-qa3-20260921'], false);
  delete config.git.deploymentEnabled['fix/individual-qa3-20260921'];
  assert.equal(config.git.deploymentEnabled['fix/individual-safe-cleanup-20260921'], false);
  ...
  assert.equal(config.git.deploymentEnabled['release/round7-verified-fixes'], false);
  delete config.git.deploymentEnabled['release/round7-verified-fixes'];
  assert.equal(config.git.deploymentEnabled['release/import-inventory-report'], false);
  delete config.git.deploymentEnabled['release/import-inventory-report'];
  assert.equal(config.git.deploymentEnabled['fix/paid-job-pricing-20260924'], false);
  delete config.git.deploymentEnabled['fix/paid-job-pricing-20260924'];
  assert.equal(config.git.deploymentEnabled['fix/audit-four-bugs-20260924'], false);
  delete config.git.deploymentEnabled['fix/audit-four-bugs-20260924'];
