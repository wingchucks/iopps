...
function fixture(){
 ...
 const db={collection:name=>({doc:id=>({id,path:name+'/'+id}),where:()=>({get:async()=>({docs:[]}),limit:()=>({get:async()=>({docs:[]})})})}),runTransaction:async callback=>callback({get:async ref=>({exists:stored.has(ref.path),data:()=>stored.get(ref.path)}),create:(ref,data)=>{assert.equal(stored.has(ref.path),false);stored.set(ref.path,data);}})};
