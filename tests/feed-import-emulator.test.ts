...
test('demo: parallel import creates exactly one job and reservation; separate locations and reposts survive', {skip:process.env.IOPPS_TEST_EMULATORS !== 'true'}, async () => {
  ...
  const base = {title:'Fictional Branch Manager', location:'Winnipeg, MB', feedId:'fixture-'+crypto.randomUUID(), employerId:'fictional', externalId:'REQ-A', publishedAt:new Date('2026-09-08'), active:true};
