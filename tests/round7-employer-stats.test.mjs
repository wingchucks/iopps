...
test('employer statistics exclude deletion tombstones and their application counts',async()=>{
 ...
 const db={collection(name){return {doc(){return {get:async()=>({exists:true,data:()=>({orgId:'fictional-org',role:'employer'})}),collection:()=>({where:()=>({get:async()=>({size:0})})})};},where(field,op,id){assert.equal(op,'==');return {get:async()=>{if(name==='jobs')return {docs:jobs,size:jobs.length};if(name==='applications'){applicationReads.push(id);return {size:1};}throw Error(name);}};}};}};
 ...
 assert.equal(response.status,200);assert.deepEqual(await response.json(),{totalPosts:3,activePosts:1,applications:3,profileViews:0});assert.deepEqual(applicationReads,['active','draft','closed']);
});
