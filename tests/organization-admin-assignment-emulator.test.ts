...
test('actual adapter: review, concurrent retries, preserved team/ownership, independent readback, duplicate identity and stale mirror', {skip:!enabled}, async t => {
  ...
  await assert.rejects(reviewAssignment(desired,store,options),/Ambiguous/);
  // Actual deployed rules contract: exact org profile access, no platform admin,
  // billing, membership rewrites, receipt writes or direct job publication.
  ...
  await clientDbSdk.updateDoc(clientDbSdk.doc(clientDb,'organizations/batc'),{description:'Profile updated by org admin'});
