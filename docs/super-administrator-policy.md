...
Before production release, verify the owner's existing Firebase Auth account is email-verified and has an admin claim. Also complete the separate database-permissions release review: existing Firestore rules still allow ordinary admins to mutate some user and organization documents directly, bypassing application APIs. The API tests are not evidence that those direct-database permissions are restricted. Live data/rules changes remain a separate release step.
