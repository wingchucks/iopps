...
The original 12 HTTP journey checks passed, and the database-permissions follow-up expands this script to 19 groups:
...
13. Member profiles, directory entries and metadata respect visibility settings;
    shared projections omit private contact details, résumés and salary preferences.
14. Profile edits cannot promote roles, relink organizations or restore suspended
    access; suspended accounts cannot create community posts through the API.
15. Applicant withdrawal is transactional and idempotent. Employers cannot mark
    an application withdrawn or reopen it, including when review and withdrawal
    requests arrive together. Private notes remain outside applicant receipts.
16. Team management stays within the owner's organization; basic team members
    cannot review applicants, and removed members lose access.
17. Message emails are derived from an existing conversation and verified Auth
    recipient rather than client-supplied addresses; duplicate requests queue once.
18. Privacy-filtered directory pagination includes members beyond the first page.
19. Self-service deletion requires recent authentication, protects Nathan's Auth
    identity and organization owners, and cannot target another account. Existing
    applications remain available to their receiving employer.
...
```sh
QA_BUILD_DIR=/absolute/path/to/credential-free-build IOPPS_TEST_EMULATORS=true GCLOUD_PROJECT=demo-iopps-preview \
...
On 2026-09-17 the preview demo was also exercised at 390px and 1280px: province
selection, Treaty 6, CPIC after an offer, paid training, Indigenous encouragement,
review, simulated publication, applicant shortlisting and reviewer notes. The
review details and teal-blue actions remained readable at both widths. These are
fictional browser interactions; backend permission checks use the separate
emulator HTTP journey. See `database-permissions-verification.md` for release gates.
