        ...
        <div className="flex h-full min-h-[500px] gap-6 p-6">
            ...
            <div className="w-1/3 space-y-4">
                <div className="flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-[var(--text-muted)] uppercase tracking-wider">Speaker Lineup</h3>
                    <span className="text-xs text-foreground0">{speakers.length} total</span>
                </div>
                ...
                <div className="space-y-2 max-h-[600px] overflow-y-auto pr-2">
                        ...
                        >
                            <div className="relative h-10 w-10 flex-shrink-0 overflow-hidden rounded-full bg-slate-700">
                                {speaker.photoUrl ? (
                                    ...
                                    <UserCircleIcon className="h-full w-full text-foreground0" />
                                )}
                            </div>
                            <div className="min-w-0 flex-1">
                                <p className={`truncate text-sm font-medium ${editingId === speaker.id ? 'text-white' : 'text-foreground'}`}>
                            ...
                            <button
                                ...
                                className="opacity-0 group-hover:opacity-100 p-1 text-foreground0 hover:text-red-400 transition-opacity"
                            >
                    ...
                    <button
                        onClick={addSpeaker}
                        className="flex w-full items-center justify-center gap-2 rounded-lg border border-dashed border-[var(--card-border)] py-3 text-sm text-[var(--text-muted)] hover:border-slate-500 hover:text-white"
                    >
            ...
            {/* Editor View */}
            <div className="flex-1 rounded-xl border border-[var(--card-border)] bg-surface p-6">
                {editingId ? (
                    (() => {
                        ...
                        return (
                            <div className="space-y-6 animate-in fade-in duration-200">
                                <div className="flex items-start gap-6">
                                    {/* Photo Upload */}
                                    <div className="group relative h-32 w-32 flex-shrink-0 overflow-hidden rounded-xl border border-[var(--card-border)] bg-surface">
                                        {speaker.photoUrl ? (
                                            <Image src={speaker.photoUrl} alt={speaker.name} fill className="object-cover" />
                                        ) : (
                                            <div className="flex h-full w-full flex-col items-center justify-center text-foreground0">
                                    ...
                                    <div className="flex-1 space-y-4">
                                        <div>
                                            <label className="text-xs uppercase text-foreground0 font-semibold mb-1 block">Full Name</label>
                                            <input
                                                type="text"
                                                value={speaker.name}
                                                onChange={(e) => updateSpeaker(speaker.id, { name: e.target.value })}
                                                className="w-full rounded bg-surface border border-[var(--card-border)] px-3 py-2 text-white focus:border-[#14B8A6] focus:outline-none"
                                                placeholder="e.g. Dr. Jane Doe"
                                            />
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="text-xs uppercase text-foreground0 font-semibold mb-1 block">Job Title</label>
                                                <input
                                                    type="text"
                                                    value={speaker.title || ""}
                                            ...
                                            <div>
                                                <label className="text-xs uppercase text-foreground0 font-semibold mb-1 block">Organization</label>
                                                <input
                                                    type="text"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs uppercase text-foreground0 font-semibold mb-1 block">Nation / Community</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="text-xs uppercase text-foreground0 font-semibold mb-1 block">LinkedIn URL</label>
                                        <input
                                            type="url"
                                ...
                                <div>
                                    <label className="text-xs uppercase text-foreground0 font-semibold mb-1 block">Biography</label>
                                    <textarea
                                        rows={6}
                                        value={speaker.bio || ""}
