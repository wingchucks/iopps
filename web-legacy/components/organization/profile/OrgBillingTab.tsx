...
export default function OrgBillingTab() {
  const { user } = useAuth();
  ...
  const hireStatus = profile?.subscription?.active ? 'active' as const : 'inactive' as const;
  ...
  const sellStatus = vendor?.subscriptionStatus === 'active' ? 'active' as const : 'inactive' as const;
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {(profile?.subscription?.active || vendor?.subscriptionStatus === 'active') && (
            <button
              ...
              className="px-4 py-2 bg-surface text-[var(--text-secondary)] rounded-lg text-sm font-medium hover:bg-surface/80 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {portalLoading ? (
        ...
        {profile?.subscription?.active ? (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="bg-surface rounded-xl p-4">
              ...
              <p className="text-xl font-bold text-[var(--text-primary)]">{jobCreditsRemaining}</p>
            </div>
            <div className="bg-surface rounded-xl p-4">
              <p className="text-sm text-foreground0 mb-1">Featured Credits</p>
              <p className="text-xl font-bold text-[var(--text-primary)]">{featuredCreditsRemaining}</p>
            </div>
          </div>
        ) : (
          <div className="bg-surface/30 rounded-xl p-6 text-center">
      ...
      {/* Module Plans */}
      <div>
        <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Module Plans</h2>
            ...
            <table className="w-full">
              ...
              <tbody>
                {payments.map((payment) => (
                  <tr key={payment.id} className="border-b border-[var(--border)]/50 hover:bg-surface/30">
                    ...
                    <td className="py-3 px-4">
                      <p className="text-sm text-[var(--text-primary)]">{payment.description}</p>
                      <p className="text-xs text-foreground0 capitalize">{payment.type.replace('_', ' ')}</p>
                    </td>
                    <td className="py-3 px-4 text-right text-sm font-medium text-[var(--text-primary)]">
      ...
      {/* Help Section */}
      <div className="bg-surface/30 border border-[var(--border)] rounded-xl p-4 flex items-center justify-between">
        <div>
        ...
        <Link
          href="/contact"
          className="px-4 py-2 bg-surface text-[var(--text-secondary)] rounded-lg text-sm font-medium hover:bg-surface/80 transition-colors"
        >
          Contact Support
        </Link>
      </div>
