  ...
  return (
    <PageShell>
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800 px-6 py-16 sm:px-12 sm:py-24 mb-12 border border-slate-700">
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          <p className="mt-4 text-lg text-slate-300 sm:text-xl">
            Explore Indigenous businesses, employers, schools, and non-profits across Canada.
      ...
      <div className="mb-8 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <span className="text-sm text-slate-400">
            {loading ? 'Loading...' : `${entries.length} ${entries.length === 1 ? 'organization' : 'organizations'} found`}
          </span>
          {hasFilters && (
            <div className="flex flex-wrap gap-2">
      ...
      {loading ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {entries.map((entry) => (
            <OrganizationCard key={entry.id} entry={entry} />
          ))}
        </div>
      )}
      ...
      {canCreateOrg && (
        <section className="mt-16 rounded-3xl bg-gradient-to-r from-slate-800 to-slate-800/50 border border-slate-700 p-8 sm:p-12 text-center">
          ...
          <p className="mt-3 text-slate-400 max-w-2xl mx-auto">
            Join our directory of Indigenous organizations. Whether you&apos;re hiring, selling products,
            offering programs, or hosting events - get discovered by our community.
          </p>
          <div className="mt-6">
            <Link
              href="/organization/onboarding"
...
function OrganizationCard({ entry }: { entry: DirectoryEntry }) {
  const Icon = ORG_TYPE_ICONS[entry.orgType];
  const ctaText = getPrimaryCTAText(entry);

  // Count total offerings
  const totalOfferings =
    entry.counts.jobsCount +
    entry.counts.programsCount +
    entry.counts.scholarshipsCount +
    entry.counts.offeringsCount +
    entry.counts.eventsCount +
    entry.counts.fundingCount;
        ...
        {entry.categories && entry.categories.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {entry.categories.slice(0, 3).map((cat) => (
              <span
                key={cat}
                className="inline-flex rounded-full bg-slate-700/50 px-2 py-0.5 text-xs text-slate-400"
              >
                {cat}
              </span>
            ))}
