...
export default async function VendorStorefrontPage({ params }: PageProps) {
  ...
  console.log(`[Shop Page] Lookup for slug "${slug}":`, {
    foundActive: !!vendor,
    vendorId: vendor?.id,
  ...
  if (!vendor) {
    vendor = await getVendorBySlugForPreview(slug);
    console.log(`[Shop Page] Preview lookup for slug "${slug}":`, {
      foundPreview: !!vendor,
      vendorId: vendor?.id,
      status: vendor?.status
    });
  ...
  if (!vendor) {
    console.log(`[Shop Page] 404 - No vendor found for slug: "${slug}"`);
    notFound();
  }
  ...
  if (!isPreviewMode) {
    incrementProfileView(vendor.id).catch((err) =>
      console.error(`[Shop Page] Failed to increment view for ${vendor?.id}:`, err)
    );
  }
