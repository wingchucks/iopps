            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="relative w-full md:w-96">
                    ...
                    <Input
                        ...
                        className="pl-10 bg-white border-slate-200 focus-visible:ring-accent/50 rounded-full"
                    />
                </div>
            </div>
                                ...
                                <CardContent className="p-4 flex items-center gap-4">
                                    ...
                                    <div className="flex-1 overflow-hidden">
                                        <Link href={`/member/${request.requesterId}`} className="block group">
                                            <h3 className="font-semibold text-slate-800 truncate group-hover:text-accent transition-colors">{request.requesterName}</h3>
                                        </Link>
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button
                                            size="sm"
                                            variant="ghost"
                                            className="h-9 px-4 text-foreground0 hover:text-slate-900"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    {orgSuggestions.length === 0 ? (
                        ...
                        orgSuggestions.map((org) => (
                            <Card key={org.id} className="bg-white border-slate-200 flex flex-col items-center p-6 text-center hover:border-accent/30 transition-all hover:bg-slate-50 group relative overflow-hidden">
            ...
            <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {suggestions.map((person) => (
                        <Card key={person.id} className="bg-white border-slate-200 overflow-hidden hover:border-accent/30 transition-all group">
                            {/* Banner - Clickable */}
                            ...
                            <CardContent className="p-4 pt-0 relative">
                                ...
                                <div className="mt-10 mb-4 space-y-1">
                                    <Link href={`/member/${person.id}`} className="block">
                                        <h3 className="font-semibold text-slate-800 line-clamp-1 group-hover:text-accent transition-colors cursor-pointer" title={person.displayName}>
                                            {person.displayName}
                                        </h3>
                                    </Link>
                                    ...
                                    {person.skills && person.skills.length > 0 && (
                                        <div className="flex flex-wrap gap-1 mt-2">
                                            {person.skills.slice(0, 3).map((skill) => (
                                                <span key={skill} className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-foreground0">
                                                    {skill}
                                                </span>
                                            ))}
                                        </div>
