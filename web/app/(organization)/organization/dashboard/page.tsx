...
/* ─── helpers ─── */
function isSchool(org: OrgData | null): boolean {
  if (!org) return false;
  return org.type === "school" || org.orgType === "SCHOOL";
}
...
function DashboardContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const dashRes = await fetch("/api/org/dashboard", { headers });
        if (!dashRes.ok) throw new Error("Dashboard fetch failed");
        const dashData = await dashRes.json();

        setOrg(dashData.org as OrgData);
        setJobs([...(dashData.jobs || [])] as DashJob[]);

        if (dashData.stats) {
          setStats(dashData.stats);
        }

        if (dashData.activity) {
  ...
  if (!loading && isSchool(org) && !org?.plan) {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        <p className="text-5xl mb-4">&#127891;</p>
        <h2 className="text-2xl font-extrabold text-text-primary mb-3">Choose Your School Plan</h2>
        ...
        <Link href="/organization/billing" className="no-underline">
          <button
            className="px-8 py-3.5 rounded-xl border-none font-bold text-base cursor-pointer transition-all hover:-translate-y-0.5"
            style={{ background: `linear-gradient(135deg, ${AMBER}, #F59E0B)`, color: "#fff", boxShadow: `0 4px 20px rgba(${AMBER_RGB},0.3)` }}
          >
            View Plans &amp; Pricing
          </button>
        </Link>
  ...
  const orgDisplayName = org?.organizationName || org?.name || "Dashboard";
              ...
              <div className="flex items-center justify-between flex-wrap gap-4 relative z-[2]">
                <div className="flex items-center gap-5">
                  <div className="relative">
                    <Avatar
                      fallback={org?.shortName || orgDisplayName}
                      size="lg"
                  ...
                  <div>
                    ...
                    <p className="text-sm mt-1" style={{ color: "var(--text-muted, #94a3b8)" }}>
                      {isSchool(org) ? "Manage your school, programs, and listings" : "Manage your organization, jobs, and applications"}
                    </p>
                    {org?.plan && (
                    ...
                    {isSchool(org) && (
                      <span className="inline-block mt-1 ml-2 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider" style={{
                ...
                <div className="flex gap-3">
                  {org?.slug && (
                    <Link href={`/organizations/${org.slug}`} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold no-underline transition-all hover:-translate-y-0.5" style={{
                  ...
                  >
                    ...
                    {isSchool(org) ? "Post a Program" : "Post a Job"}
                  </button>
                </div>
              </div>
            </div>
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <DashCard>
          <div className="flex items-center justify-between mb-5">
            <span className="text-base font-bold" style={{ color: "var(--text, #f8fafc)" }}>Profile Views &mdash; 30 Days</span>
      ...
      <DashCard>
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/organization/jobs/new" className="px-3 py-1.5 rounded-lg text-xs font-semibold no-underline transition-all hover:-translate-y-0.5" style={{
      ...
      {profileSub === "General" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <DashCard>
            ...
            <GlowButton disabled={saving} onClick={() => saveProfile({ name: profileForm.name, organizationName: profileForm.name, tagline: profileForm.tagline, description: profileForm.description })}>
          ...
          <div className="flex flex-col gap-4">
            <DashCard>
              ...
              <div className="flex items-center gap-4 mb-4">
                <Avatar fallback={org?.shortName || org?.organizationName || org?.name || ""} size="lg" src={org?.logoUrl || org?.logo} />
          ...
          <div className="flex flex-wrap gap-2 p-3.5 rounded-xl mb-5 min-h-[52px] items-center transition-all" style={{
            ...
            {tags.map((tag) => (
              <span key={tag} className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-medium" style={{
                ...
                {tag}
                <span className="cursor-pointer opacity-50 hover:opacity-100 text-sm" onClick={() => setTags((prev) => prev.filter((t) => t !== tag))}>&times;</span>
              </span>
            ))}
            <input
...
/* ===============================================================
   BILLING TAB
   =============================================================== */
function BillingTab({ org }: { org: OrgData | null }) {
  return (
    <>
      <h2 className="text-xl font-extrabold tracking-tight mb-5" style={{
        background: "linear-gradient(135deg, var(--text, #f8fafc), var(--text-sec, #cbd5e1))",
      ...
      <DashCard>
        <div className="text-center py-8">
          ...
          {isSchool(org) && (
            <p className="text-xs mb-4" style={{ color: "var(--text-muted, #64748b)" }}>
              Single program: <strong>$50</strong> &middot; Full School Plan: <strong>$5,500/year</strong>
            </p>
          )}
          <Link href="/organization/billing" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold no-underline transition-all hover:-translate-y-0.5" style={{
...
function LoadingSkeleton() {
  return (
    <div className="flex flex-col gap-4">
      <div className="h-32 rounded-[20px] animate-pulse bg-surface" />
      <div className="flex gap-2">
        {Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-9 w-24 rounded-xl animate-pulse bg-surface" />)}
      </div>
      <div className="grid grid-cols-4 gap-4">
        {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-28 rounded-2xl animate-pulse bg-surface" />)}
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div className="h-60 rounded-2xl animate-pulse bg-surface" />
        <div className="h-60 rounded-2xl animate-pulse bg-surface" />
      </div>
    </div>
  );
