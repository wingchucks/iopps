...
export default function ConferenceBillingTab() {
    const { user } = useAuth();
    const [paidConferences, setPaidConferences] = useState<Conference[]>([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        if (!user) return;

        const fetchPaidConferences = async () => {
            try {
                ...
                setPaidConferences(conferences);
            } catch (error) {
    ...
    const calculateTotalSpent = () => {
        return paidConferences.reduce((total, conf) => {
            return total + (conf.amountPaid || 0);
        }, 0) / 100; // Convert cents to dollars
    };

    const getActiveConferencesCount = () => {
        return paidConferences.filter(conf => {
            if (!conf.expiresAt) return false;
            let expiryDate: Date;
            const expiresAt = conf.expiresAt;
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-[var(--card-border)] text-left text-sm font-semibold text-[var(--text-muted)]">
                                    ...
                                    <th className="pb-3">Listed Date</th>
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        ...
                        <ul className="mt-2 space-y-2 text-sm text-[var(--text-secondary)]">
                            ...
                            <li className="flex items-start gap-2">
                                <span className="text-blue-500">•</span>
                                <span>Premium listings ($150) get priority placement for 90 days</span>
                            </li>
                            <li className="flex items-start gap-2">
                                <span className="text-blue-500">•</span>
                                <span>Featured conferences ($350) appear at top with spotlight placement for 120 days</span>
                            </li>
                            <li className="flex items-start gap-2">
                                <span className="text-blue-500">•</span>
