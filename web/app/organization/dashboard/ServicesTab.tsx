...
export default function ServicesTab() {
    ...
    if (loading) return <div className="text-[var(--text-muted)] p-4">Loading services...</div>;
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-[var(--text-muted)]">Manage the services you offer to the community.</p>
                </div>
                <button
            ...
            {services.length === 0 ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {services.map(service => (
                        <div key={service.id} className="group relative overflow-hidden rounded-xl border border-[var(--card-border)] bg-surface p-5 hover:border-blue-500/50 transition-colors">
                            <div className="flex justify-between items-start mb-2">
                                <span className="px-2 py-0.5 rounded text-xs font-medium bg-blue-500/10 text-blue-400">
                                    {service.category}
                                </span>
                                <div className="flex gap-2">
                                    <button onClick={() => handleDuplicate(service)} className="text-[var(--text-muted)] hover:text-blue-400" title="Duplicate">
                                        <DocumentDuplicateIcon className="h-4 w-4" />
                                    </button>
                                    <button onClick={() => handleOpenModal(service)} className="text-[var(--text-muted)] hover:text-white" title="Edit">
                                        <PencilSquareIcon className="h-4 w-4" />
                                    </button>
                                    <button onClick={() => handleDelete(service.id, service.title)} className="text-[var(--text-muted)] hover:text-red-400" title="Delete">
                                        <TrashIcon className="h-4 w-4" />
                                    </button>
                                </div>
                            </div>
                            <h3 className="font-semibold text-white mb-1 line-clamp-1">{service.title}</h3>
                            <p className="text-sm text-[var(--text-muted)] line-clamp-2 mb-3">{service.tagline}</p>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <div className="w-full max-w-2xl rounded-2xl bg-surface border border-[var(--card-border)] shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={handleSave} className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Service Title</label>
                                <input
                                    required
                                    className="w-full rounded-lg bg-surface border border-[var(--card-border)] px-3 py-2 text-white focus:border-blue-500 focus:outline-none"
                                    value={formData.title}
                                    onChange={e => setFormData({ ...formData, title: e.target.value })}
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Tagline (for cards)</label>
                                <input
                                    required
                                    className="w-full rounded-lg bg-surface border border-[var(--card-border)] px-3 py-2 text-white focus:border-blue-500 focus:outline-none"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Price Range</label>
                                    <input
