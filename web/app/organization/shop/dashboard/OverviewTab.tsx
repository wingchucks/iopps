    ...
    <div className="space-y-8">
      ...
      {profile && (
        <div className={`rounded-2xl p-4 ${
          profile.approvalStatus === 'approved' || profile.status === 'active'
            ? 'border border-emerald-500/30 bg-emerald-500/10'
            : profile.approvalStatus === 'pending_review' || profile.status === 'draft'
            ? 'border border-amber-500/30 bg-amber-500/10'
            : profile.approvalStatus === 'rejected' || profile.status === 'suspended'
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-400">
                {profile.approvalStatus === 'approved' || profile.status === 'active'
                  ? 'Your shop is visible to the public in Shop Indigenous.'
                  : profile.approvalStatus === 'pending_review' || profile.status === 'draft'
                  ? 'Your shop is being reviewed. You can still preview it using the link below.'
                  : profile.approvalStatus === 'rejected' || profile.status === 'suspended'
            ...
            {profile.slug && (
              <Link
                ...
                className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-800/50 px-4 py-2 text-sm font-medium text-slate-300 transition hover:border-emerald-500/50 hover:text-emerald-400"
              >
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                </svg>
                {profile.approvalStatus === 'approved' || profile.status === 'active' ? 'View Shop' : 'Preview Shop'}
              </Link>
            )}
          </div>
        </div>
          ...
          {(profile?.slug || user?.uid) && profile?.businessName ? (
            ...
            >
              ...
              <h4 className="font-semibold text-white group-hover:text-purple-400">
                {isPublished ? 'View Your Shop' : 'Preview Shop'}
              </h4>
              <p className="mt-1 text-sm text-slate-400">
                {isPublished
            ...
            <button
              ...
              className="group rounded-xl border border-purple-500/30 bg-purple-500/10 p-6 text-left transition-all hover:border-purple-500/50 hover:bg-purple-500/20"
            >
              <div className="mb-2 text-2xl">
                <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="mt-1 text-sm text-slate-400">
                Complete your profile to create your public shop
              </p>
            </button>
          )}
        </div>
