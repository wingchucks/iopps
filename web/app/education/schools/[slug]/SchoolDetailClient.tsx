  ...
  return (
    <FeedLayout activeNav="education" fullWidth>
      ...
      <nav className="mb-8 text-sm text-foreground0">
        ...
        <span className="mx-2">→</span>
        <Link href="/education/schools" className="hover:text-[var(--text-primary)] transition-colors">
          Schools
        </Link>
        <span className="mx-2">→</span>
        <span className="text-[var(--text-primary)]">{school.name}</span>
      </nav>

      {/* School Header */}
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] overflow-hidden mb-8">
        {/* Banner Image */}
        {school.bannerUrl && (
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          {/* Logo/Icon */}
          <div className={`flex h-24 w-24 items-center justify-center rounded-2xl bg-accent/20 border border-[#14B8A6]/40 shrink-0 ${school.bannerUrl ? '-mt-16 bg-[var(--card-bg)] shadow-lg' : ''}`}>
            {school.logoUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
          ...
          <div className="flex-1">
            ...
            <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-2">{school.name}</h1>
      ...
      {/* Tabs */}
      <div className="mb-8 flex gap-2 overflow-x-auto border-b border-[var(--border)] pb-px">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            ...
            className={`whitespace-nowrap rounded-t-lg px-4 py-3 text-sm font-medium transition-all ${activeTab === tab.id
              ...
              : "border-b-2 border-transparent text-foreground0 hover:border-[var(--border)] hover:text-[var(--text-secondary)]"
              }`}
          >
            {tab.label}
            {tab.count !== null && (
        ...
        <div className="space-y-8">
          ...
          {school.description && (
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] p-6">
              ...
              <p className="text-[var(--text-secondary)] whitespace-pre-wrap">{school.description}</p>
            </div>
          )}
          ...
          {school.indigenousServices && (
            <div className="rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] p-6">
              <h2 className="text-xl font-bold text-[var(--text-primary)] mb-4">Indigenous Student Services</h2>
              <div className="grid gap-4 sm:grid-cols-2">
                {school.indigenousServices.studentCentre && (
                  <div className="flex items-start gap-3">
                    <span className="text-[#14B8A6]">✓</span>
                    <span className="text-[var(--text-secondary)]">{school.indigenousServices.studentCentre.name || "Indigenous Student Centre"}</span>
                  </div>
                )}
                {school.indigenousServices.elderInResidence && (
                  <div className="flex items-start gap-3">
                    <span className="text-[#14B8A6]">✓</span>
                    <span className="text-[var(--text-secondary)]">Elder-in-Residence Program</span>
                  </div>
                )}
                {school.indigenousServices.ceremonySpace && (
                  <div className="flex items-start gap-3">
                    <span className="text-[#14B8A6]">✓</span>
                    <span className="text-[var(--text-secondary)]">Dedicated Cultural Spaces</span>
                  </div>
                )}
                {school.indigenousServices.psychologists && (
                  <div className="flex items-start gap-3">
                    <span className="text-[#14B8A6]">✓</span>
                    <span className="text-[var(--text-secondary)]">Indigenous Counselling Services</span>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {school.social.facebook && (
                    <a href={school.social.facebook} target="_blank" rel="noopener noreferrer"
                       className="flex items-center gap-2 rounded-lg bg-surface px-3 py-2 text-sm text-[var(--text-secondary)] hover:bg-surface transition-colors">
                      <span>📘</span> Facebook
                    </a>
                  )}
              ...
              {programs.map((program) => (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className="rounded-md bg-surface px-2 py-1 text-xs text-[var(--text-secondary)] capitalize">
                      {program.level}
                    </span>
                  </div>
                  <h3 className="font-bold text-[var(--text-primary)] mb-2 group-hover:text-[#14B8A6] transition-colors line-clamp-2">
                    {program.name}
                  </h3>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {events.map((event) => (
                ...
                >
                  ...
                  <h3 className="font-bold text-[var(--text-primary)] mb-2 group-hover:text-[#14B8A6] transition-colors">
                    {event.name}
                  </h3>
