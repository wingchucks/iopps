...
export default function SchoolDetailClient({ school: initialSchool, error: initialError }: SchoolDetailClientProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    async function loadRelatedData() {
      ...
      // Check if saved
      if (user) {
        const saved = await isSchoolSaved(user.uid, school.id);
        setIsSaved(saved);
      }
    }
                        ...
                        <span>📅 Deadline: {
                          typeof scholarship.deadline === 'string'
                            ...
                            : 'seconds' in scholarship.deadline
                              ? new Date((scholarship.deadline as { seconds: number }).seconds * 1000).toLocaleDateString()
                              : new Date(scholarship.deadline as Date).toLocaleDateString()
                        }</span>
                      )}
                    </div>
                  </div>
                  <span className="text-sm font-semibold text-[#14B8A6]">View &rarr;</span>
                </Link>
              ))}
            </div>
          ) : (
                    ...
                    📅 {event.startDatetime && (
                      typeof event.startDatetime === 'string'
                        ...
                        : 'seconds' in event.startDatetime
                          ? new Date((event.startDatetime as { seconds: number }).seconds * 1000).toLocaleDateString()
                          : 'TBD'
                    )}
                  </p>
                </Link>
