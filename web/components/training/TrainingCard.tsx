...
export default function TrainingCard({ program, featured = false }: TrainingCardProps) {
    const getFormatBadgeColor = (format: string) => {
        switch (format.toLowerCase()) {
            ...
            case 'in-person': return 'bg-emerald-500/20 text-emerald-300';
    ...
    return (
        <div className={`group flex flex-col overflow-hidden rounded-2xl border transition-all hover:-translate-y-1 ${featured
            ...
            : "border-slate-700 bg-slate-800/50 hover:border-teal-500/30"
            }`}>
            {/* Header / Banner */}
            <div className="relative px-5 py-4 border-b border-slate-700/50 bg-slate-800/40">
                ...
                <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm text-slate-300 mb-5">
                    ...
                    {program.startDate && (
                        <div className="flex items-center gap-1.5 col-span-2">
                            <span className="text-slate-500 text-xs uppercase tracking-wide">Starts:</span>
