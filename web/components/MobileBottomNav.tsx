...
export function MobileBottomNav() {
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-[var(--card-border)] bg-background/95 backdrop-blur-lg md:hidden safe-area-inset-bottom">
                    ...
                    return (
                        <Link
                            ...
                            className={`flex flex-1 flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-medium transition-colors ${isActive
                                ? "text-accent"
                                : "text-[var(--text-muted)] active:text-foreground"
                                }`}
                        >
