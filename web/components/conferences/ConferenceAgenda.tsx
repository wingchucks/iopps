...
const sessionTypeConfig: Record<string, { icon: ReactNode; color: string; label: string }> = {
  ...
  ceremony: {
    ...
    color: "text-[#14B8A6] bg-[#14B8A6]/10 border-[#14B8A6]/30",
    label: "Ceremony",
  },
  other: {
    icon: (
...
function SessionCard({
  ...
  return (
    <div
      className={`rounded-xl border bg-slate-900/50 transition-all ${typeConfig.color.split(" ")[2]} hover:border-opacity-60`}
    >
      <button
      ...
      >
        ...
        <div className="flex-shrink-0 text-center">
          <p className="text-sm font-bold text-slate-200">{session.time}</p>
          {session.endTime && (
        ...
        <div className="flex-1 min-w-0">
          ...
          <h4 className="mt-2 font-semibold text-slate-100">{session.title}</h4>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              {sessionSpeakers.map((speaker) => (
                <button
                  ...
                  className="flex items-center gap-2 rounded-full border border-slate-700 bg-slate-800/50 px-2 py-1 text-xs text-slate-300 transition-colors hover:border-[#14B8A6] hover:text-[#14B8A6]"
                >
                  {speaker.photoUrl ? (
                    <img
                    ...
                    <div className="flex h-5 w-5 items-center justify-center rounded-full bg-slate-700 text-[10px] font-medium">
                      {speaker.name
                        .split(" ")
                        .map((n) => n[0])
                        .join("")
  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-[#08090C] p-6 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-200">Schedule</h2>
