...
const TIERS = [
    ...
    { id: "community", label: "Community", color: "bg-accent/20 border-accent" },
] as const;
    ...
    return (
        <div className="p-6 space-y-8">
            {TIERS.map((tier) => {
                ...
                return (
                    <div key={tier.id} className="rounded-xl border border-[var(--card-border)] bg-slate-900/30 p-4">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button
                                ...
                                className="text-xs flex items-center gap-1 text-[var(--text-muted)] hover:text-white"
                            >
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {tierSponsors.length === 0 && (
                                <div className="col-span-full py-8 text-center text-sm text-foreground0 border border-dashed border-[var(--card-border)] rounded-lg">
                                    No {tier.label.toLowerCase()} sponsors yet.
                                </div>
                            )}
                            {tierSponsors.map((sponsor) => (
                                <div key={sponsor.id} className="relative group rounded-lg border border-[var(--card-border)] bg-surface p-4 transition-all hover:border-[var(--card-border)]">
                                    <button
                                        onClick={() => removeSponsor(sponsor.id)}
                                        className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 p-1 text-foreground0 hover:text-red-400 transition-opacity z-10"
                                    >
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded bg-white p-1">
                                            {sponsor.logoUrl ? (
                                                ...
                                                <PhotoIcon className="h-full w-full text-[var(--text-secondary)]" />
                                            )}
                                        ...
                                        <div className="flex-1 space-y-2">
                                            <input
                                                ...
                                                className="w-full rounded bg-surface border border-[var(--card-border)] px-2 py-1 text-sm text-white focus:border-[#14B8A6] focus:outline-none"
                                            />
                                            <input
                                                type="url"
                                                ...
                                                placeholder="https://"
                                                className="w-full rounded bg-surface border border-[var(--card-border)] px-2 py-1 text-xs text-[var(--text-muted)] focus:border-[#14B8A6] focus:outline-none"
                                            />
                                        </div>
                                    </div>
                                </div>
