...
export default function BusinessCard({ post, onSave, saved }: Props) {
  return (
    <div className="card-interactive border border-[var(--card-border)] bg-[var(--card-bg)] rounded-xl p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          {post.orgLogoURL ? (
            ...
            <div className="w-10 h-10 rounded-lg bg-[var(--surface-raised)] flex items-center justify-center flex-shrink-0 text-lg">🏪</div>
          )}
      ...
      {post.description && (
        <p className="text-sm text-[var(--text-secondary)] mt-3 line-clamp-2">{post.description}</p>
      )}
    </div>
  );
}
