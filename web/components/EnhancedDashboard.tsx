...
const SidebarItem = ({ icon: Icon, label, active = false, badge = 0 }: any) => (
    <div className={`flex items-center justify-between px-4 py-3 rounded-xl transition-all cursor-pointer mb-1 ${active
        ...
        {badge > 0 && (
            <span className="bg-[#14B8A6] text-slate-950 text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[20px] text-center">
                {badge}
            </span>
        )}
    </div>
...
const StatCard = ({ icon: Icon, value, label }: any) => (
    <div className="bg-[#08090C] border border-slate-800/80 p-6 rounded-2xl hover:border-[#14B8A6]/50 transition-all group">
        <div className="flex items-center gap-4 mb-2">
            <div className="p-2.5 rounded-xl bg-slate-900/50 text-slate-400 group-hover:text-[#14B8A6] transition-colors">
                <Icon size={20} />
            </div>
            <div>
...
export default function EnhancedDashboard() {
    ...
    return (
        <div className="min-h-screen bg-[#020617] text-slate-200 font-sans selection:bg-[#14B8A6]/30">
            {/* Background Glows */}
            ...
            <div className="relative max-w-[1600px] mx-auto flex h-screen gap-6 p-6 overflow-hidden">
                ...
                <aside className="w-72 flex flex-col gap-6 flex-shrink-0">
                    {/* Logo & Org Info */}
                    <div className="bg-[#08090C] border border-slate-800/60 p-5 rounded-3xl flex items-center gap-4 backdrop-blur-xl">
                        ...
                        <div>
                            <h2 className="font-bold text-slate-50">IOPPS Indigenous</h2>
                            <p className="text-xs text-slate-500">Red Pheasant First Nation</p>
                        </div>
                    </div>

                    {/* Binary Toggle Switch */}
                    <div className="bg-[#08090C] border border-slate-800/60 p-5 rounded-3xl backdrop-blur-xl">
                        <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 mb-4 px-1">Dashboard Mode</h3>

                        <div
                            onClick={toggleMode}
                            className="relative h-12 bg-slate-900/50 rounded-full border border-slate-800 cursor-pointer p-1 flex items-center justify-between select-none shadow-inner"
                        >
                            {/* Sliding Knob */}
                            <div
                            ...
                            {/* Labels */}
                            <div className={`relative z-20 w-1/2 text-center text-xs font-bold transition-colors duration-300 flex items-center justify-center gap-2 ${activeMode === 'employer' ? 'text-white' : 'text-slate-500'
                                }`}>
                                <Briefcase size={14} />
                                Employer
                            </div>
                            <div className={`relative z-20 w-1/2 text-center text-xs font-bold transition-colors duration-300 flex items-center justify-center gap-2 ${activeMode === 'vendor' ? 'text-slate-950' : 'text-slate-500'
                                }`}>
                                Vendor
                                <ShoppingBag size={14} />
                            </div>
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        <div>
                            <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 mb-4 px-4">Account</h3>
                ...
                <main className="flex-1 overflow-y-auto rounded-3xl pb-20 scroll-smooth">
                    {/* Header */}
                    <header className="flex items-center justify-between mb-8 sticky top-0 bg-slate-950/20 backdrop-blur-lg p-2 -mx-2 z-10">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${activeMode === 'employer' ? 'bg-blue-600/20 text-blue-400' : 'bg-[#14B8A6]/20 text-[#14B8A6]'
                                    }`}>
                        ...
                        <button className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm shadow-xl transition-all active:scale-95 ${activeMode === 'employer'
                            ...
                            : 'bg-[#14B8A6] text-slate-950 shadow-[#14B8A6]/20 hover:bg-[#16cdb8]'
                            }`}>
                            <Plus size={18} />
                    ...
                    {/* Main Card View */}
                    <div className="bg-[#08090C] border border-slate-800/60 rounded-3xl p-8 backdrop-blur-xl">
                        ...
                        <div className="space-y-4">
                            {[1, 2, 3].map(i => (
                                <div key={i} className="flex items-center justify-between p-5 rounded-2xl bg-slate-900/40 border border-slate-800/50 hover:border-[#14B8A6]/30 transition-all cursor-pointer group">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center text-slate-400 group-hover:bg-[#14B8A6]/10 group-hover:text-[#14B8A6] transition-all">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">3 days ago</span>
                                        <span className={`px-4 py-1.5 rounded-lg text-xs font-bold ${i === 1 ? 'bg-blue-600/20 text-blue-400' : 'bg-slate-800 text-slate-400'
                                            }`}>
                                            {i === 1 ? 'New' : 'Reviewed'}
                                        </span>
                                    </div>
