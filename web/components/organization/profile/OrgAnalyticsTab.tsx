...
function aggregateByMonth(items: Record<string, unknown>[], dateField: string) {
  const monthCounts = new Map<string, number>();

  items.forEach(item => {
    const date = item[dateField] as { seconds?: number } | string | undefined;
    if (!date) return;
    ...
    try {
      ...
      const applicationsLast30Days = applications.filter(app => {
        ...
        const createdDate = new Date(
          (app.createdAt as { seconds: number }).seconds
            ? (app.createdAt as { seconds: number }).seconds * 1000
            : (app.createdAt as unknown as string)
        );
      ...
      const jobsByMonth = aggregateByMonth(jobs as unknown as Record<string, unknown>[], 'createdAt');
      const applicationsByMonth = aggregateByMonth(applications as unknown as Record<string, unknown>[], 'createdAt');
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Jobs Posted (Last 6 Months)</h2>
        ...
        {/* Application Trends */}
        <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Applications (Last 6 Months)</h2>
      ...
      {/* Two Column Layout: Top Jobs & Status */}
      <div className="grid gap-6 lg:grid-cols-2">
        {/* Top Performing Jobs */}
        <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Top Performing Jobs</h2>
                ...
                <div key={job.jobId} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link
                      ...
                      className="text-sm font-medium text-[var(--text-primary)] hover:text-accent truncate block"
                    >
                      {job.title}
                    </Link>
        ...
        {/* Application Status Breakdown */}
        <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-4">Applications by Status</h2>
          {analytics.applicationsByStatus.length > 0 ? (
            <div className="space-y-4">
              {analytics.applicationsByStatus.map((item) => {
                ...
                const barColor = colors[item.status] || 'from-slate-500 to-slate-600';
