"use client";
import BusinessProfileDemo from "./BusinessProfileDemo";
...
].map(job => ({...job,hiringDetails:normalizeHiringDetails({})}));
const initialPeople = [
...
export default function EmployerDemo() {
  ...
  const businessMode = params.get("workspace") === "business";
  const [hiringDetails, setHiringDetails] = useState<HiringDetails>(() => normalizeHiringDetails({}));
  const [step, setStep] = useState(0);
  const [title, setTitle] = useState("");
  const [locationCity, setLocationCity] = useState("");
  const [locationProvince, setLocationProvince] = useState("");
  const [workArrangement, setWorkArrangement] = useState("On-site");
  const location = [workArrangement,formatJobLocation(locationCity,locationProvince)].filter(Boolean).join(" · ");
  const [description, setDescription] = useState("");
  const [requirements, setRequirements] = useState("");
  const navigate = (next: string) => router.push(`/demo/employer?tab=${encodeURIComponent(next)}${businessMode ? "&workspace=business" : ""}`, { scroll: false });
  const saveJob = (status: string) => {
    setJobs(current => [{id: `demo-${Date.now()}`, hiringDetails, title, location, status, applicationCount: 0, createdAt: "2026-09-16"}, ...current]);
    setNotice(status === "draft" ? "Demo draft saved for this visit." : "Demo job marked active. Nothing was published publicly.");
    setTitle(""); setLocationCity(""); setLocationProvince(""); setWorkArrangement("On-site"); setDescription(""); setRequirements(""); setStep(0); setHiringDetails(normalizeHiringDetails({})); navigate("Jobs");
  };
  return <div className="employer-workspace min-h-screen" style={{background:"var(--bg)", color:"var(--text)"}}>
    ...
    <div className="max-w-[1100px] mx-auto px-4 py-7 md:px-10">
      <div className="flex justify-between gap-4 mb-5"><Link href="/" className="font-bold">IOPPS</Link><button onClick={() => {setJobs(initialJobs); setPeople(initialPeople); setCompany("Prairie Pathways — Demo Company"); setNotice("Demo reset."); setProfileReset(value => value + 1); setHiringDetails(normalizeHiringDetails({})); setStep(0); setTitle(""); setLocationCity(""); setLocationProvince(""); setWorkArrangement("On-site"); setDescription(""); setRequirements(""); navigate("Overview");}} className="underline text-sm">Reset demo</button></div>
      <header className="employer-hero rounded-[20px] p-7 mb-6"><p className="text-xs uppercase tracking-widest mb-2">{businessMode ? "Example business account" : "Example employer account"}</p><h1 className="text-2xl md:text-3xl font-bold">{company}</h1><p className="mt-2">{businessMode ? "Showcase your work, build your profile, and explore hiring from one account." : "Explore your hiring workspace. Try creating a job and reviewing sample candidates."}</p></header>
      <div className="flex flex-wrap gap-4 mb-5"><Link href="/demo/employer" aria-current={!businessMode?"page":undefined}>Hiring workspace</Link><Link href="/demo/employer?workspace=business" aria-current={businessMode?"page":undefined}>Business promotion workspace</Link></div>
      <nav className="employer-tabs flex gap-2 mb-6" aria-label="Demo employer navigation">{tabs.map(item => <button key={item} onClick={() => navigate(item)} aria-current={tab === item ? "page" : undefined} className="button-gradient-soft px-4 py-2 rounded-xl border border-[var(--border)]">{item}</button>)}</nav>
      {notice && <p role="status" className="employer-note mb-5">{notice}</p>}
      {tab === "Overview" && businessMode && <BusinessOverview demo publicHref="/demo/employer?workspace=business&tab=Profile&view=public" />}
      {tab === "Overview" && !businessMode && <EmployerOverview demo statsAvailable stats={{totalPosts:jobs.length,activePosts:jobs.filter(j=>j.status === "active").length,applications:people.length,profileViews:48}} jobs={jobs} activity={[{id:"sample-activity",type:"application",message:"Sample Candidate A applied for Community Engagement Coordinator.",timestamp:"2026-09-16"}]} timeAgo={() => "Sample activity"} formatTimestamp={value => `Sample posting · ${String(value)}`} />}
      {tab === "Jobs" && <section className="employer-panel"><div className="employer-section-heading"><h2>Your demo jobs</h2><button className="employer-primary" onClick={() => navigate("Post a Job")}>+ Post a job</button></div>{jobs.map(job => <div key={job.id} className="employer-job-row"><div><label className="block text-xs mb-1" htmlFor={`title-${job.id}`}>Job title</label><input id={`title-${job.id}`} className="demo-input" value={job.title} onChange={e=>setJobs(current=>current.map(j=>j.id===job.id?{...j,title:e.target.value}:j))} /><p>{job.location} · {job.applicationCount} sample applications</p><HiringDetailsSummary value={job.hiringDetails} /></div><label className="text-xs">Posting status<select className="demo-input mt-1" value={job.status} onChange={e=>{setJobs(current=>current.map(j=>j.id===job.id?{...j,status:e.target.value}:j));setNotice("Demo status updated. No real job was changed.");}}><option value="draft">Draft</option><option value="active">Active</option><option value="closed">Closed</option></select></label></div>)}</section>}
      ...
      {tab === "Post a Job" && <section className="employer-panel"><h2>Create a demo job</h2><ol className="flex flex-wrap gap-4 my-5 text-sm">{["Job Details","Requirements","Review & Publish"].map((label,index)=><li key={label} aria-current={index===step?"step":undefined} className={index===step?"font-bold text-[var(--teal)]":""}>{index+1}. {label}</li>)}</ol><form onSubmit={e=>{e.preventDefault(); if(step<2)setStep(step+1);else saveJob("active");}}>
        {step===0 && <div className="grid gap-4"><label>Job title<input required className="demo-input mt-2" value={title} onChange={e=>setTitle(e.target.value)} /></label><label>Work arrangement<select className="demo-input mt-2" value={workArrangement} onChange={e=>setWorkArrangement(e.target.value)}><option>On-site</option><option>Hybrid</option><option>Remote</option></select></label><JobLocationFields required city={locationCity} province={locationProvince} onChange={(city,province)=>{setLocationCity(city);setLocationProvince(province);}} /><label>About the role<textarea required className="demo-input mt-2" value={description} onChange={e=>setDescription(e.target.value)} rows={4} /></label></div>}
        {step===1 && <><HiringDetailsFields value={hiringDetails} onChange={setHiringDetails} /><label className="block">Requirements<textarea className="demo-input mt-2" value={requirements} onChange={e=>setRequirements(e.target.value)} rows={5} placeholder="Skills and experience for this example role" /></label></>}
        {step===2 && <div className="employer-note"><h3>{title}</h3><p>{location}</p><p className="whitespace-pre-wrap my-3">{description}</p><p className="whitespace-pre-wrap">{requirements}</p><HiringDetailsSummary value={hiringDetails} /><strong>This demonstration will not publish a real job.</strong></div>}
        <div className="flex flex-wrap gap-3 mt-6">{step>0 && <button type="button" className="px-4 py-3" onClick={()=>setStep(step-1)}>Back</button>}<button type="button" disabled={!title.trim()} className="button-gradient-soft px-4 py-3 border rounded-xl disabled:opacity-40" onClick={()=>saveJob("draft")}>Save demo draft</button><button type="submit" className="employer-primary">{step===2?"Simulate publishing":"Continue →"}</button></div></form></section>}
      <div hidden={tab !== "Profile"}><BusinessProfileDemo key={profileReset} publicView={params.get("view") === "public"} onNameSaved={setCompany} /></div>
    </div>
  </div>;
}
