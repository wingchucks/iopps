...
export default function BusinessReviewDemo() {
  ...
  return <main className="min-h-screen bg-[#f4f7f8] px-4 py-6 text-[#153347] sm:px-8 sm:py-10">
    <div className="mx-auto max-w-6xl">
      ...
      <nav className="mb-6 flex flex-wrap gap-2" aria-label="Preview perspective">{[["business", "Business owner"], ["admin", "Admin review"], ["public", "Public directory"]].map(([key, name]) => <button type="button" key={key} aria-pressed={view === key} onClick={() => { setView(key); setNotice(""); }} className={`min-h-11 rounded-xl px-4 py-3 text-sm font-bold ${view === key ? "bg-[#006b62] text-white" : "border border-slate-300 bg-white text-slate-700"}`}>{name}</button>)}<button type="button" onClick={() => { setOrg({ ...fixture, directoryReview: newBusinessListingReview() }); setView("business"); setDescription(String(fixture.description)); setEditing(false); setNotice("Demo reset."); }} className="ml-auto min-h-11 px-3 text-sm font-semibold underline">Reset demo</button></nav>
