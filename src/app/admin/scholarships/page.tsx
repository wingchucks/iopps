            ...
            {!loading && filteredScholarships.length > 0 && (
              <div className="flex items-center justify-between pt-4 border-t border-[var(--card-border)]">
                ...
                <div className="flex gap-2">
                  <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="px-3 py-1.5 text-sm rounded-lg border border-[var(--card-border)] button-gradient-soft hover:bg-accent/10 disabled:opacity-40">Previous</button>
                  <span className="text-sm px-3 py-1.5">{currentPage} / {totalPages}</span>
                  <button disabled={currentPage >= totalPages} onClick={() => setCurrentPage(p => p + 1)} className="px-3 py-1.5 text-sm rounded-lg border border-[var(--card-border)] button-gradient-soft hover:bg-accent/10 disabled:opacity-40">Next</button>
                </div>
              </div>
            )}
          </CardContent>
