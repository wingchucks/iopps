...
import { EXISTING_LISTING_REVIEW, LISTING_REVIEW_STATUSES, REVIEW_LABELS, type ListingReviewStatus } from "@/lib/business-listing-review";
import BusinessReviewPanel, { type ListingReviewItem, type ReviewAction } from "@/components/business-review/BusinessReviewPanel";

type QueueView = ListingReviewStatus | "location";

export default function BusinessReviewsPage() {
  const { user } = useAuth();
  const [status, setStatus] = useState<QueueView>("pending");
  ...
  async function decide(action: ReviewAction, feedback: string) {
    // Existing public listings predate review; admins can still send them back for changes.
    const review = item?.review ?? (item?.isPublic ? EXISTING_LISTING_REVIEW : null);
    ...
    const response = await fetch("/api/admin/business-reviews", {
      ...
      body: JSON.stringify({ orgId: item.org.id, revision: review.revision, status: review.status, action, feedback }),
    });
    const result = await response.json();
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <div className="flex flex-wrap items-end gap-4"><label className="text-sm font-semibold">Review status<select value={status} onChange={event => { setStatus(event.target.value as QueueView); setItems([]); setSelected(null); setNotice(""); }} className="mt-2 block min-h-11 rounded-xl border border-slate-300 bg-white px-4 py-2 text-slate-900">{LISTING_REVIEW_STATUSES.map(value => <option key={value} value={value}>{REVIEW_LABELS[value]}</option>)}<option value="location">Public listings: location to check</option></select></label><button type="button" disabled={busy} onClick={() => load()} className="button-gradient-soft min-h-11 rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold disabled:opacity-50">Refresh queue</button></div>
