  ...
  return (
    <AppShell>
      <div className="min-h-screen bg-bg">
        <ScholarshipDetailContent scholarship={scholarship} org={org} related={related} />
      </div>
    </AppShell>
  );
}
...
function ScholarshipDetailContent({
  ...
  useEffect(() => {
    if (scholarship && user) {
      isPostSaved(user.uid, scholarship.id)
        .then(setSaved)
        .catch(() => {});
    }
  }, [scholarship, user]);
        ...
        <div className="md:col-span-2">
          ...
          {scholarship.requirements && scholarship.requirements.length > 0 && (
            <>
              ...
              <ul className="mb-6 pl-0 list-none">
                {scholarship.requirements.map((r: string, i: number) => (
          ...
          {scholarship.badges && scholarship.badges.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-6">
              {scholarship.badges.map((b: string) => (
                <span
                  key={b}
