...
export default function BusinessListingStatus({ org, onSubmit, onEdit, onRefresh }: {
  ...
  async function refresh() {
    ...
    setBusy(true); setError(""); setRefreshState("refreshing");
    try { await onRefresh(); setRefreshState("done"); }
    catch (err) { setRefreshState("idle"); setError(err instanceof Error ? err.message : "Unable to refresh. Please try again."); }
  ...
  return <section className="mb-6 rounded-2xl border border-[#b9ddd8] bg-[#f1faf8] p-5 text-[#173d43] sm:p-6" aria-label="Business listing status">
    ...
    <div className="mt-4 flex flex-wrap gap-3">
      ...
      {onRefresh && <button type="button" disabled={busy} onClick={refresh} className="min-h-11 px-2 py-3 text-sm font-bold underline disabled:opacity-50">{refreshState === "refreshing" ? "Refreshing…" : "Refresh status"}</button>}
    </div>
    {correctionNeeded && <p className="mt-3 text-sm">Save your corrections before resubmitting.</p>}
    {refreshState !== "idle" && <p role="status" className="mt-3 text-sm">{refreshState === "refreshing" ? "Refreshing listing status…" : "Status refreshed. The current listing status is shown above."}</p>}
