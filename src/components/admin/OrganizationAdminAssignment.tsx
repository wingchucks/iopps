...
export default function OrganizationAdminAssignment({orgId,getToken,onApplied}: {
  ...
  return <section className="mt-5 border-t border-border pt-5" aria-labelledby="assignment-title">
    ...
    <form className="mt-4 space-y-3" onSubmit={event=>{event.preventDefault();void requestReview();}}>
      ...
      <button ref={reviewButton} className="rounded-lg bg-teal px-4 py-2 font-semibold text-white disabled:opacity-50" disabled={busy || !!review || !role} type="submit">{busy && !review ? "Reviewing…" : "Review assignment"}</button>
    </form>
    ...
    <dialog ref={dialog} aria-labelledby="assignment-review-title" className="m-auto max-h-[90dvh] w-[calc(100%-2rem)] max-w-2xl overflow-y-auto rounded-2xl border border-border bg-card p-5 text-text backdrop:bg-black/60" onCancel={e=>{e.preventDefault();close();}}>
      {review && <>
        ...
        <div className="mt-4 flex flex-wrap justify-end gap-3">
          <button type="button" onClick={close} disabled={busy} className="rounded-lg border border-border px-4 py-2 disabled:opacity-50">Cancel</button>
          <button type="button" onClick={()=>void apply()} disabled={busy || confirmation !== review.confirmation} className="rounded-lg bg-teal px-4 py-2 font-semibold text-white disabled:opacity-50">{busy ? "Applying and verifying…" : "Confirm assignment"}</button>
        </div>
      </>}
    </dialog>
  </section>;
