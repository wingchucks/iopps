...
// A first free listing waits for IOPPS; these statuses are set by the review, never by the owner.
const REVIEW_STATUSES = ["pending", "changes_requested", "rejected"];
const STATUS_LABELS: Record<string, string> = { active: "Published", closed: "Closed", pending: "In review", changes_requested: "Changes requested", rejected: "Not approved" };
const inView = (status: string | undefined, view: string) => view === "all" || (view === "review" ? REVIEW_STATUSES.includes(status || "") : status === view);
...
export default function OpportunityManager({ kind, getToken, demo = false, initialNew = false, demoInitial = [] }: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setReviewFirst(payload.reviewFirst === true);
  ...
  async function persist(data: Record<string, unknown>, status: OpportunityStatus, initial?: OpportunityRecord): Promise<OpportunityRecord> {
    if (demo) {
  ...
  async function save(data: Record<string, unknown>, status: OpportunityStatus) {
    const saved = await persist(data, status, editing && editing !== "new" ? editing : undefined);
    setEditing(null); setRequestId(""); setFilter("all");
    setMessage(`${demo ? "Demo: " : ""}${saved.status === "pending" ? "Thanks. IOPPS reviews an organization’s first free listing before it appears publicly. It stays private until approved, and after that your listings publish right away." : saved.status === "draft" ? "Private draft saved. It is not listed publicly." : "Listing published. You can edit or close it below."}`);
  }
  ...
  const filtered = items.filter(item => inView(item.status, filter));
  return <div className="min-w-0 rounded-2xl bg-slate-50 p-4 text-slate-900 sm:p-6">
    {editing ? <OpportunityEditor key={editing === "new" ? "new" : editing.id} kind={kind} initial={editing === "new" ? undefined : editing} demo={demo} getToken={getToken} reviewFirst={reviewFirst} onSave={save} onCancel={() => { setEditing(null); setRequestId(""); }} /> : <>
      ...
      <div className="my-6 flex flex-wrap gap-2" aria-label="Listing status">{[["all", "All"], ["draft", "Drafts"], ["review", "In review"], ["active", "Published"], ["closed", "Closed"]].filter(([value]) => value !== "review" || items.some(item => inView(item.status, "review"))).map(([value, label]) => <button key={value} aria-pressed={filter === value} className={filter === value ? opportunityPrimary : opportunityButton} onClick={() => setFilter(value)}>{label} <span className="ml-2 opacity-80">{items.filter(item => inView(item.status, value)).length}</span></button>)}</div>
      {loading ? <p role="status" className="py-8 text-sm">Loading your listings…</p> : !loadError && !filtered.length ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-7 text-center"><h2 className="text-lg font-bold">{filter === "all" ? "Your next opportunity starts here." : "No listings in this view."}</h2><p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-slate-600">Save a draft while you gather the details. Publish when you are ready for people to find it.</p></div> : <div className="space-y-3">{filtered.map(item => <article key={item.id} className="flex min-w-0 flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0"><span className={`inline-block rounded-full px-2.5 py-1 text-xs font-bold ${item.status === "active" ? "bg-teal-50 text-teal-800" : REVIEW_STATUSES.includes(item.status || "") ? "bg-amber-50 text-amber-900" : "bg-slate-100 text-slate-600"}`}>{STATUS_LABELS[item.status || ""] || "Private draft"}</span><h2 className="mt-2 break-words text-lg font-bold">{item.title}</h2>{item.status === "pending" && <p className="mt-1 text-sm text-slate-600">Private until IOPPS approves it.</p>}{typeof item.reviewFeedback === "string" && item.reviewFeedback && item.status !== "pending" && <p className="mt-2 whitespace-pre-wrap break-words rounded-xl bg-amber-50 p-3 text-sm text-amber-950"><strong>Feedback from IOPPS: </strong>{item.reviewFeedback}</p>}<p className="mt-1 text-sm text-slate-600">{event ? getEventDisplayDates(item) || "Date to be confirmed" : isJobRecordExpired(item) ? "Intake closed — update the deadline for the next intake" : item.deadline ? `Deadline: ${item.deadline}` : "Confirm deadline with provider"}</p></div>
        <div className="flex shrink-0 flex-wrap gap-2">{item.status !== "rejected" && <button className={opportunityButton} onClick={() => { setEditing(item); setMessage(""); }}>Edit<span className="sr-only"> {item.title}</span></button>}{["draft", "closed"].includes(item.status || "") && <button disabled={!!busy} className={opportunityButton} onClick={() => void remove(item)}>{busy === item.id ? "Deleting…" : "Delete listing"}<span className="sr-only"> {item.title}</span></button>}{item.status === "active" && <>{!demo && <Link className={opportunityButton} href={`/${kind}/${item.slug || item.id}`}>View public listing</Link>}<button disabled={!!busy} className={opportunityButton} onClick={() => void close(item)}>{busy === item.id ? "Closing…" : "Close listing"}</button></>}</div>
      </article>)}</div>}
    </>}
  </div>;
}
