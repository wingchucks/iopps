...
import { eligibilityLabel, eligibleInProvince, scholarshipEligibility } from "@/lib/scholarship-eligibility";
...
/** For scholarships the province filter means where applicants may live or study; null means not stated. */
function provinceMatch(item: OpportunityRecord, province: string, events: boolean): boolean | null {
  if (!province) return true;
  return events ? opportunityProvince(item) === province : eligibleInProvince(scholarshipEligibility(item), province);
}
...
export default function OpportunityDirectory({ kind }: { kind: OpportunityKind }) {
  ...
  const filters = useMemo(() => ({ search, category, events, dateFilter, deadlineFilter }), [search, category, events, dateFilter, deadlineFilter]);
  const filtered = useMemo(() => items.filter(item => matchesOtherFilters(item, filters) && provinceMatch(item, province, events) === true)
    .sort((a, b) => events ? (getEventStartDate(a)?.getTime() || Infinity) - (getEventStartDate(b)?.getTime() || Infinity) : Number(isJobRecordExpired(a)) - Number(isJobRecordExpired(b))), [items, filters, province, events]);
  // Listings that don't say where applicants must live are neither included nor hidden: they are shown separately.
  const unstated = useMemo(() => !province || events ? [] : items.filter(item => matchesOtherFilters(item, filters) && provinceMatch(item, province, events) === null), [items, filters, province, events]);
  const provinceName = CANADIAN_PROVINCES.find(([code]) => code === province)?.[1] ?? province;
  ...
  const renderCard = (item: OpportunityRecord) => {
        ...
        return <article key={item.id} className="flex min-w-0 flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
          ...
          <h3 className="mt-4 break-words text-xl font-extrabold leading-snug"><Link className="block hover:text-teal-800 focus-visible:outline-2 focus-visible:outline-teal-700" href={`/${kind}/${item.slug || item.id}`}>{item.title}</Link></h3>
          ...
          {events ? location && <p className="mt-1 break-words text-sm text-slate-600">{location}</p> : <p className="mt-1 break-words text-sm text-slate-600">{eligibilityLabel(scholarshipEligibility(item))}</p>}
    ...
    <div className="mx-auto max-w-6xl px-4 py-7 sm:px-8 sm:py-9">
      <section aria-label="Find opportunities" className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1fr]">
          ...
          <label className="min-w-0 text-xs font-bold uppercase tracking-wide text-slate-600">{events ? "Province or territory" : "Where you live or study"}<select value={province} onChange={e => setProvince(e.target.value)} className={selectClass}><option value="">All provinces & territories</option>{CANADIAN_PROVINCES.map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></label>
