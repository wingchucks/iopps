...
export default function OpportunityDirectory({ kind }: { kind: OpportunityKind }) {
  ...
  const filtered = useMemo(() => items.filter(item => {
    // Closed intakes leave browse at once; their pages stay reachable from links and saved items.
    if (!events && (item.intakeClosed === true || isJobRecordExpired(item))) return false;
