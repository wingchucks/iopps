...
export function EmployerOverview({ stats, statsAvailable, activity, jobs, timeAgo, formatTimestamp, demo = false }: {
  ...
  return <>
    ...
    <div className="employer-actions">
      <Link href={demo ? `/demo/employer?tab=Jobs` : "/org/dashboard?tab=Jobs"}><span>01 / JOBS</span><h3>{drafts.length ? `${drafts.length} draft${drafts.length === 1 ? "" : "s"} to finish` : "Manage your openings"}</h3><p>Edit roles, review posting status, and keep listings current.</p><strong>Manage jobs →</strong></Link>
    ...
    <div className="employer-columns">
      <section className="employer-panel"><div className="employer-section-heading"><h2>Recent jobs</h2><Link href={demo ? `/demo/employer?tab=Jobs` : "/org/dashboard?tab=Jobs"}>View all →</Link></div>
