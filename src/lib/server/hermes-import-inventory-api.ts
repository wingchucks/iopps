...
export const IMPORT_INVENTORY_PATH = '/api/hermes/v1/reports/import-inventory';
export const IMPORT_INVENTORY_BODY = '{"report":"import-inventory-v1"}';
export interface ImportInventoryDeps extends HermesMachineAuthDeps {
  reportKeyIds: readonly string[];
...
/** Fixed, signed, bounded request. Only the injected read-only adapter can read inventory. */
export async function handleImportInventoryRequest(request: Request, deps: ImportInventoryDeps): Promise<Response> {
  try {
    const url = new URL(request.url);
    if (request.method !== 'POST' || url.pathname !== IMPORT_INVENTORY_PATH || url.search || url.hash) {
    ...
    let body: string;
    try {
      if (raw[0] === 0xef && raw[1] === 0xbb && raw[2] === 0xbf) throw new Error('BOM');
      body = new TextDecoder('utf-8', { fatal: true }).decode(raw);
      if (!Buffer.from(body, 'utf8').equals(Buffer.from(raw))) throw new Error('UTF-8');
    } catch { return json({ ok: false, error: 'Invalid report body' }, 400); }
    const keyId = request.headers.get('x-hermes-key-id') ?? '';
    if (!deps.reportKeyIds.includes(keyId)) return json({ ok: false, error: 'Report key not authorized' }, 403);
    ...
    if (body !== IMPORT_INVENTORY_BODY) return json({ ok: false, error: 'Invalid report contract' }, 400);
    const report = await deps.readInventory();
    return json({ ok: true, report });
  } catch { return json({ ok: false, error: 'Import inventory unavailable' }, 503); }
}
