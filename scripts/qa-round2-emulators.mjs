...
const command=process.argv.includes('--public-audit') ? `"${process.execPath}" scripts/qa-public-audit-integration.mjs` : process.argv.includes('--safe-cleanup') ? `"${process.execPath}" scripts/qa-safe-cleanup-integration.mjs` : process.argv.includes('--individual3') ? `"${process.execPath}" scripts/qa-individual3-integration.mjs` : process.argv.includes('--individual3-browser') ? `"${process.execPath}" scripts/qa-individual3-integration.mjs --browser-only` : process.argv.includes('--employer-browser') ? `"${process.execPath}" --import "${new URL('./test-typescript-loader.mjs',import.meta.url).href}" scripts/qa-employer-browser.mjs` : process.argv.includes('--employer-suite') ? `"${process.execPath}" scripts/qa-employer-integration-run.mjs` : process.argv.includes('--browser-only') ? `"${process.execPath}" --import "${new URL('./test-typescript-loader.mjs',import.meta.url).href}" scripts/qa-round2-browser.mjs` : `"${process.execPath}" scripts/qa-round2-integration-run.mjs`;
