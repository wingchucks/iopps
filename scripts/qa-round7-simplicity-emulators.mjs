...
const out=path.resolve('reports/round7-simplicity');fs.mkdirSync(out,{recursive:true});
if(process.argv.includes('--inside')){
 ...
 if(built){run('typecheck',['node_modules/typescript/bin/tsc','--noEmit','--incremental','false']);run('built-browser',['scripts/qa-round7-simplicity-browser.mjs']);}
 ...
 const command=`"${process.execPath}" scripts/qa-round7-simplicity-emulators.mjs --inside ${process.argv.includes('--browser-only')?'--browser-only':''}`;
